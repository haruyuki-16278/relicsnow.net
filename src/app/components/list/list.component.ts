...
export class ListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.httpClient.get('assets/articles/article-list.json').subscribe((response) => {
  ...
  getDateLabel(article: Article): string {
    const createdAt = `${article.created_at.getFullYear()}/${
      article.created_at.getMonth() + 1
    }/${article.created_at.getDate()}`;
    let updatedAt: string | undefined = undefined;
    if (article.updated_at)
      updatedAt = `${article.updated_at.getFullYear()}/${
        article.updated_at.getMonth() + 1
